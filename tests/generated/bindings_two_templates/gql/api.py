...
class GetHighlightBound[TResult: pydantic.BaseModel](runtime.GQLBoundOperation):
    async def execute(self, *, id: builtins.str) -> TResult:
        return await API_CLIENT.query(
            _API_GQL_CAST("type[TResult]", GetHighlightResult),
            self.exec_source,
            variables={"id": id, **self.fragment_args},
            headers=self.headers,
            slot_readers=self.slot_readers,
...
class GetHighlight(runtime.GQLTemplate):
    @overload
    def bind(self, *, highlight: Sequence[Never] = ()) -> GetHighlightBound[GetHighlightResult[Never]]: ...
    @overload
    def bind[TModelHighlight: pydantic.BaseModel, TReadsHighlight](self, *, highlight: OnImageAttachment[TModelHighlight, TReadsHighlight]) -> GetHighlightBound[GetHighlightResult[OnImageAttachment[TModelHighlight, TReadsHighlight] | TReadsHighlight]]: ...
    def bind(self, *, highlight: slots.GQLBindableFragment[pydantic.BaseModel, Any] | Sequence[slots.GQLBindableFragment[pydantic.BaseModel, Any]] = ()) -> runtime.GQLBoundOperation:
        if _API_GQL_BIND_DISPATCH.get(slots.dispatch_key('GetHighlight', {'highlight': highlight})) is None:
            raise LookupError("unknown bind combination for GetHighlight; single-fragment and empty combinations are generated from the schema, so this is a tuple combination no call site writes literally - write it, then regenerate the package. A call whose template is an expression the scan cannot follow is never read either: those are listed, with the reason, in the debug run's ignored_binds.json")
        return GetHighlightBound[GetHighlightResult].bound__(
            _API_GQL_BIND_DISPATCH[slots.dispatch_key('GetHighlight', {'highlight': highlight})], {'highlight': slots.as_bindable_fragments(highlight)},
        )
...
_API_GQL_BIND_DISPATCH: dict[slots.DispatchKey, runtime.BoundSpec] = {
    ...
    # See: queries.py:22
    ('GetHighlight', ()): ('query GetHighlight($id: ID!) {\n  post(id: $id) {\n    id\n    highlight {\n      __typename\n    }\n  }\n}', {"highlight": ()}),
    # See: queries.py:22, queries.py:3
    ('GetHighlight', (('highlight', (ImageParts,)),)): ('query GetHighlight($id: ID!) {\n  post(id: $id) {\n    id\n    highlight {\n      __typename\n      ...ImageParts\n    }\n  }\n}\n\nfragment ImageParts on ImageAttachment {\n  url\n}', {"highlight": ((ImageParts, frozenset({'ImageAttachment'})),)}),
}
