from __future__ import annotations
# Code generated by iron_gql, DO NOT EDIT.
# fmt: off
# pyright: reportUnusedImport=false
# ruff: noqa


import datetime
from collections.abc import AsyncGenerator
from contextlib import AbstractAsyncContextManager
from typing import Annotated
from typing import Literal
from typing import overload

import pydantic

from iron_gql import runtime

import pydantic.alias_generators

import builtins

from tests.generated.websockets_codegen.settings import GRAPHQL_URL


API_CLIENT = runtime.GQLClient(
    base_url=GRAPHQL_URL,
)


class GQLModel(pydantic.BaseModel):
    model_config = pydantic.ConfigDict(
        populate_by_name=True,
        serialize_by_alias=True,
        alias_generator=pydantic.AliasGenerator(
            validation_alias=pydantic.alias_generators.to_camel,
            serialization_alias=pydantic.alias_generators.to_camel,
        ),
        extra="forbid",
        validate_default=True,
    )


class Event(GQLModel):
    id: builtins.str
    message: str


class EventsResult(GQLModel):
    events: Event


class Events(runtime.GQLOperation):
    # See: queries.py:3
    def execute(self, *, channel: str) -> AbstractAsyncContextManager[AsyncGenerator[EventsResult]]:
        return API_CLIENT.subscribe(
            EventsResult,
            'subscription Events($channel: String!) {\n  events(channel: $channel) {\n    id\n    message\n  }\n}',
            variables={"channel": channel},
            headers=self.headers,
        )


@overload
def api_gql(stmt: Literal['\n    subscription Events($channel: String!) {\n        events(channel: $channel) {\n            id\n            message\n        }\n    }\n    ']) -> Events: ...
@overload
def api_gql(stmt: str) -> runtime.GQLOperation: ...


_API_GQL_DISPATCH: dict[str, type[runtime.GQLOperation]] = {
    '\n    subscription Events($channel: String!) {\n        events(channel: $channel) {\n            id\n            message\n        }\n    }\n    ': Events,
}


def api_gql(stmt: str) -> runtime.GQLOperation:
    query_cls = _API_GQL_DISPATCH.get(stmt)
    if query_cls is not None:
        return query_cls()
    return runtime.GQLOperation()
